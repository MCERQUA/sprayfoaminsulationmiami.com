...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${service.title} in Miami`,
    description: service.description,
    keywords: service.keywords ? [...service.keywords] : undefined,
    alternates: { canonical: url },
    openGraph: {
      title: `${service.title} in Miami | Miami Spray Foam`,
      description: service.description,
      url,
    },
  };
    ...
    <>
      ...
      <main>
          ...
          <div className="container-wide">
            ...
            <div className="grid lg:grid-cols-12 gap-10 items-center">
              <FadeIn className="lg:col-span-7">
                <div className="inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-clay-gradient text-white mb-5 shadow-warm">
                  <Icon className="h-7 w-7" strokeWidth={2.2} />
                </div>
                <span className="pill-sage">Insulation service</span>
                ...
                {detail && (
                  <p className="mt-5 lead max-w-2xl">{detail.heroBlurb}</p>
                )}
              ...
              <FadeIn direction="left" className="lg:col-span-5">
                <div className="rounded-t-[10rem] rounded-b-3xl overflow-hidden border-4 border-white shadow-warm-lg">
                  <img
                    src="/images/attic-foam.jpg"
                    alt={`${service.title} — professional spray foam installation in Miami`}
            ...
            <div className="container-wide grid lg:grid-cols-3 gap-8 lg:gap-10">
              <FadeIn>
                <div className="rounded-3xl bg-white border border-adobe shadow-card p-7 h-full">
                  <div className="flex items-center gap-2.5 mb-5">
                    <span className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-clay/10 text-clay">
                      <ShieldCheck className="h-5 w-5" strokeWidth={2.2} />
                    </span>
                    <h2 className="font-heading font-bold text-xl text-espresso">What&apos;s included</h2>
                  </div>
                  <ul className="space-y-3">
                    {detail.whatsCovered.map((c) => (
                      <li key={c} className="flex items-start gap-2.5 text-cocoa text-[0.95rem] leading-relaxed">
              ...
              <FadeIn delay={0.08}>
                <div className="rounded-3xl bg-espresso text-cream p-7 h-full">
                  <div className="flex items-center gap-2.5 mb-5">
                    <span className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-white/10 text-gold">
                      <Users className="h-5 w-5" strokeWidth={2.2} />
                    </span>
                    <h2 className="font-heading font-bold text-xl text-cream">Who it&apos;s for</h2>
                  </div>
                  <ul className="space-y-3">
                    {detail.whoItsFor.map((c) => (
                      <li key={c} className="flex items-start gap-2.5 text-cream/85 text-[0.95rem] leading-relaxed">
              ...
              <FadeIn delay={0.16}>
                <div className="rounded-3xl bg-white border border-adobe shadow-card p-7 h-full">
                  <div className="flex items-center gap-2.5 mb-5">
                    <span className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-gold/15 text-gold-dark">
                      <Target className="h-5 w-5" strokeWidth={2.2} />
                    </span>
                    <h2 className="font-heading font-bold text-xl text-espresso">Why Miami Spray Foam</h2>
                  </div>
                  <ul className="space-y-3">
                    {detail.whyCca.map((c) => (
                      <li key={c} className="flex items-start gap-2.5 text-cocoa text-[0.95rem] leading-relaxed">
        ...
        <FAQ
          ...
          title={
            <>
              ...
              <span className="text-clay">{service.title.toLowerCase()}</span>
            </>
          }
          background="sand"
        />
        ...
        <section className="bg-cream py-16">
          <div className="container-wide">
            <h2 className="font-heading font-bold text-2xl md:text-3xl text-espresso mb-8">
              ...
              {related.map((r) => {
                ...
                return (
                  <Link
                    ...
                    className="group p-6 rounded-3xl bg-white border border-adobe shadow-card hover:shadow-card-hover hover:-translate-y-1 transition-all duration-300"
                  >
                    <div className="inline-flex h-11 w-11 items-center justify-center rounded-2xl bg-clay/10 text-clay group-hover:bg-clay group-hover:text-white transition-colors mb-3">
                      <RIcon className="h-6 w-6" strokeWidth={2.2} />
                    </div>
