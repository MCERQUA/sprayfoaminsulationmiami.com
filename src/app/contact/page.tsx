...
export default function ContactPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await fetch(WEBHOOK_URL, {
        ...
        body: JSON.stringify({ form_name: "contact", source: "sprayfoaminsulationmiami.com", ...formData }),
      });
      setSubmitted(true);
    } catch {
  ...
  const contactCards = [
    ...
    { icon: MessageCircle, label: "Get an estimate", value: "Start online", href: "/quote", sub: "Same-day response guaranteed" },
  ];
          ...
          <div className="container-tight text-center">
            <FadeIn>
              ...
              <p className="mt-5 lead max-w-2xl mx-auto">
                Questions or a free estimate — reach a person who knows Miami insulation, not a queue.
              </p>
            </FadeIn>
          </div>
        </section>
            ...
            <FadeIn>
              <div className="grid sm:grid-cols-1 gap-4 mb-8">
                {contactCards.map((c) => (
                  <a
                    key={c.label}
                    href={c.href}
                    className="group flex items-center gap-4 p-5 rounded-2xl bg-white border border-adobe shadow-card hover:shadow-card-hover hover:-translate-y-0.5 transition-all"
                  >
                    <span className="flex-shrink-0 inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-clay/10 text-clay group-hover:bg-clay group-hover:text-white transition-colors">
              ...
              <div className="rounded-2xl bg-espresso text-cream p-6">
                <div className="flex items-start gap-3 mb-4">
                  ...
                  <div>
                    <p className="font-heading font-bold text-cream">Office</p>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-heading font-bold text-cream">Hours</p>
                    <p className="text-cream/75 text-sm">{SITE.hours}</p>
                    <p className="text-cream/55 text-xs mt-0.5">Same-day response guaranteed</p>
                  </div>
                </div>
              </div>
            </FadeIn>
                ...
                >
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input id="phone" name="phone" type="tel" value={formData.phone} onChange={handleChange} placeholder="844-967-5247" className={inputClass} />
                    </div>
                  </div>

                  <div>
                    <label htmlFor="email" className={labelClass}>Email *</label>
                    <input id="email" name="email" type="email" required value={formData.email} onChange={handleChange} placeholder="jane@smithfoam.com" className={inputClass} />
                  </div>

                  <div>
                    <label htmlFor="subject" className={labelClass}>Subject</label>
                    <input id="subject" name="subject" type="text" value={formData.subject} onChange={handleChange} placeholder="How can we help?" className={inputClass} />
                  </div>

                  <div>
                    <label htmlFor="message" className={labelClass}>Message *</label>
                    <textarea id="message" name="message" rows={5} required value={formData.message} onChange={handleChange} placeholder="Tell us about your project or question…" className={`${inputClass} resize-none`} />
                  </div>
