...
import {
  ShieldCheck, Award, CalendarCheck, Timer, Home, BadgeCheck,
} from "lucide-react";
import { CREDENTIALS } from "@/lib/site";
import { FadeIn } from "@/components/animations/FadeIn";

const ICONS = {
  MapPin: ShieldCheck, CalendarCheck, HardHat: Award, Timer, Zap: Timer, Award: BadgeCheck,
  Home, BadgeCheck, ShieldCheck,
} as const;
      ...
      <div className="container-wide py-6 md:py-8">
        <FadeIn>
          <ul className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-x-4 gap-y-5">
            {CREDENTIALS.map((c) => {
              const Icon = ICONS[c.icon as keyof typeof ICONS] ?? FALLBACK_ICON;
              return (
