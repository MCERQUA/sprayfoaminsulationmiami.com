  ...
  return (
    <header
      className={cn(
        ...
        scrolled
          ? "bg-cream/90 backdrop-blur-md border-b border-adobe shadow-sm"
          : "bg-transparent"
      )}
    >
      <nav className="container-wide flex items-center justify-between h-18 py-3">
        {/* Logo */}
        <Link href="/" className="flex items-center gap-2.5 group" aria-label="Miami Spray Foam home">
        ...
        <div className="hidden lg:flex items-center gap-1">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="px-4 py-2 rounded-full text-sm font-heading font-semibold text-cocoa hover:text-clay hover:bg-clay/8 transition-colors"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="hidden lg:flex items-center gap-3">
          <a
            href={SITE.phoneHref}
            className="inline-flex items-center gap-2 text-sm font-heading font-bold text-espresso hover:text-clay transition-colors"
          >
        ...
        {/* Mobile toggle */}
        <button
          className="lg:hidden inline-flex items-center justify-center h-11 w-11 rounded-xl bg-white border border-adobe shadow-card text-espresso"
      ...
      <div
        className={cn(
          "lg:hidden fixed inset-0 top-[72px] bg-cream transition-all duration-300",
        ...
        <div className="container-wide py-8 flex flex-col gap-2">
          {NAV_LINKS.map((link, i) => (
            <Link
              ...
              className="px-5 py-4 rounded-2xl text-lg font-heading font-bold text-espresso hover:bg-sand transition-colors"
