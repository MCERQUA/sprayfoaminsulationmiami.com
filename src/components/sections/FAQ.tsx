...
export function FAQ({
  ...
  const resolvedTitle = title ?? (
    <>
      ...
      <span className="text-clay">spray foam insulation in Miami</span>.
    </>
  );
              ...
              <FadeIn key={item.q} delay={i * 0.04}>
                <div
                  className={cn(
                    ...
                    isOpen
                      ? "bg-white border-clay/30 shadow-card"
                      : "bg-white/70 border-adobe hover:bg-white"
                  )}
                >
                  <button
                  ...
                  >
                    <span className="font-heading font-bold text-espresso text-base md:text-lg leading-snug">
                      {item.q}
                    </span>
                    <span
                      className={cn(
                        "flex-shrink-0 inline-flex h-8 w-8 items-center justify-center rounded-full transition-colors",
                        isOpen ? "bg-clay text-white" : "bg-sand text-clay"
                      )}
                    >
                  ...
                  >
                    <div className="overflow-hidden">
                      <p className="px-6 pb-6 text-mocha leading-relaxed">{item.a}</p>
                    </div>
                  </div>
                </div>
              </FadeIn>
